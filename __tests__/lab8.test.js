describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // Once you have the innerText property, use innerText['_remoteObject'].value to get the text value of it
     ...
     let addToCartBut = await shadow.$('button');
     
     await addToCartBut.click();

     let innerText = await addToCartBut.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      ...
      let addToCartButton = await shadow.$('button');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // assert cart state remains after page reload by checking all items are still in cart
    for(let i = 0; i < prodItems.length; i++){
      let shadow = await prodItems[i].getProperty('shadowRoot');
      let addToCartButton = await shadow.$('button');
      let innerText = await addToCartButton.getProperty('innerText');

      expect(innerText['_remoteObject'].value).toBe('Remove from Cart');
    }

    // assert cart count is still 20
    let cartCount = await page.$('#cart-count')
    let innerText = await cartCount.getProperty('innerText');
    expect(innerText['_remoteObject'].value).toBe('20') 
